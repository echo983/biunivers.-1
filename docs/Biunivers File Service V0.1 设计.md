...
文档状态：V0.3 实现基线的历史快照。文件对象、RefStore 和资源能力设计仍有效；其中管理员
token 相关描述记录当时实现，当前控制入口以 V0.14.1 单一主人控制模型和现行 runbook 为准。

## 1. 文档状态

- 状态：已实施、验收并归档为 Biunivers `v0.3.0`
...
- [x] 冻结 PVLog Core WASM ABI v1 和 128 MiB 线性内存上限；大规模性能基准作为后续
  优化工作，不阻断 V0.1 文件能力闭环。
...
- Entry 索引；（已完成）
- 新建、读取、保存、移动、重命名和删除；（底层事务与内容读取已完成）
- 新建文件与更新内容的原子发布；（已完成）
- 流式分片；（固定分片、流式读取和有界流式写入已完成，HTTP 路由待接入）
- Checkpoint；
- 冲突和重试。
- 环境配置、显式初始化、正常恢复与离线降级状态；（已完成）
...
- 保存对话框；（目录导航、文件名校验和待保存句柄已完成；未成功 PUT 不创建空文件）
- File HTTP API。（GET/PUT 流式路由、实例认证、精确 CORS、版本复核和原子写入已完成）

### 阶段 4：记事本验收

- 独立第三方记事本；（已发布并通过真实安装验收）
- 打开、保存和另存为；（已通过真实浏览器与 R2 验收）
- 冲突提示；（旧句柄覆盖保护、`FILE_VERSION_CONFLICT` 和记事本可见提示已完成）
- 重启恢复；（已验证容器重启后 revision 和文件内容保持一致）
- 失败链路；（对象存储写入失败不发布 Ref、也不产生可见空文件，已自动化验证）
- Docker 人工验收。（首轮通过）
- 64 MiB 分片边界真实验收。（64 MiB + 1 已完成流式上传和读回）

首轮真实验收记录见
[`acceptance/File Service V0.1 首轮验收.md`](acceptance/File%20Service%20V0.1%20首轮验收.md)。

### 阶段 5：维护

- SQLite 在线一致性备份与独立恢复演练；（已完成）
- GC 扫描和只读报告；（已完成；无删除路径）
- 真实 R2 GC 验收；（全部 16 个协议对象可达、0 个候选，已完成）
- 损坏检查；（启动、备份、恢复及完整内容扫描已完成）
- 性能与容量测试。（64 MiB + 1 分片边界已完成；更大规模基准推迟到后续版本）
